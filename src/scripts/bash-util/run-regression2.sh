...
for YEAR in 1964 1996; do
	...
	for WITH_GPC in false true; do
		...
		CLOSE_VARS="AM HOST CLOSE_WIDE"

		run_test "$YEAR" "$COV_TYPE" "$WITH_GPC" "true" "false" "$CLOSE_VARS"  # sep-host
		run_test "$YEAR" "$COV_TYPE" "$WITH_GPC" "true" "true" "$CLOSE_VARS"   # sep-host-close

		CLOSE_VARS="AM HOST CLOSE_GMT1"
		...
		run_test "$YEAR" "$COV_TYPE" "$WITH_GPC" "true" "false" "AM HOST PRE POST"
...
find "$BASE_OUT" -maxdepth 1 -type f -regextype posix-extended -regex '.*sep-host1.sep-close1.*(\+CG1).*' -exec mv {} "$TIMESTAMP_DIR/Am-Host-Pre-Post-Close-gmt1_sep-host-close/" \;
find "$BASE_OUT" -maxdepth 1 -type f -regextype posix-extended -regex '.*sep-host1.sep-close0.*(\+CG1).*' -exec mv {} "$TIMESTAMP_DIR/Am-Host-Pre-Post-Close-gmt1_sep-host/" \;
find "$BASE_OUT" -maxdepth 1 -type f -regextype posix-extended -regex '.*sep-host1.sep-close1.*(\+Cwd).*' -exec mv {} "$TIMESTAMP_DIR/Am-Host-Pre-Post-Close-wide_sep-host-close/" \;
find "$BASE_OUT" -maxdepth 1 -type f -regextype posix-extended -regex '.*sep-host1.sep-close0.*(\+Cwd).*' -exec mv {} "$TIMESTAMP_DIR/Am-Host-Pre-Post-Close-wide_sep-host/" \;
find "$BASE_OUT" -maxdepth 1 -type f -regextype posix-extended -regex '.*sep-host0.sep-close1.*(\+C)[^+_].*' -exec mv {} "$TIMESTAMP_DIR/Am-Host-Pre-Post-Close_sep-close/" \;
find "$BASE_OUT" -maxdepth 1 -type f -regextype posix-extended -regex '.*sep-host0.sep-close0.*(\+C)[^+_].*' -exec mv {} "$TIMESTAMP_DIR/Am-Host-Pre-Post-Close/" \;
